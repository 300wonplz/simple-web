...
<html lang="en">
  ...
  <?php include("../import/modal.php");?>
        ...
        <script>
          $(function(){
            document.getElementById("signerrormessage").innerHTML = "쓰기 권한이 없습니다.\n관리자에게 문의하세요.";
            $("#signuperror").modal();
      ...
      if($_SESSION['health']<15){
        ...
        <script>
          $(function(){
            document.getElementById("signerrormessage").innerHTML = "체력을 다 소모하였습니다.";
            $("#signuperror").modal();
            $("#ok").on('click', function(e){
              location.href = "board.php";
            })
  ...
  <body class="nav-md">
    ...
    function postForm() {
              ...
              try{
                var url ='../board/freeboard_submit.php';
                ...
                $.ajax({
                  ...
                  success: function(res) {
                    if(res)
                    {
                      ...
                      if(result[0] == "login_error")
                      {
                        alert("로그인 후 이용 가능합니다.\n 로그인 페이지로 이동합니다");
                        location.href="../login/login.php";
                      }
                      else if(result[0] == "health_error_nofile"){
                        document.getElementById("signerrormessage").innerHTML = "체력이 부족합니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "no_title"){
                        document.getElementById("signerrormessage").innerHTML = "제목이 없습니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "no_comment"){
                        document.getElementById("signerrormessage").innerHTML = "내용이 없습니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "health_error_file"){
                        document.getElementById("signerrormessage").innerHTML = "파일 업로드할 체력이 부족합니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "exist_error file"){
                        document.getElementById("signerrormessage").innerHTML = result[1] + " 파일은 이미 존재하는 파일입니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "filetype_error file"){
                        document.getElementById("signerrormessage").innerHTML = result[1] + " 은 허용되지 않는 확장자입니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "filesize_error file"){
                        document.getElementById("signerrormessage").innerHTML = result[1] + " 파일이 제한용량을 초과하였습니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "length_error file"){
                        document.getElementById("signerrormessage").innerHTML = result[1] + " 의 제목이 제한길이를 초과하였습니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "totalsize_error"){
                        document.getElementById("signerrormessage").innerHTML = "총 파일 용량은 1Mbyte 입니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == 'fail'){
                        document.getElementById("signerrormessage").innerHTML = "작성 실패";
                        $('#signuperror').modal();
                      }

                      else{
                        document.getElementById("signerrormessage").innerHTML = "작성 완료";
                        $('#signuperror').modal();
                        $("#ok").on('click', function(e){
