...
<html lang="en">
  <head>
    ...
    /* 페이지 접근권한 확인*/
      ...
      // DB에서 정보 가져오기
      $result = mysqli_query($mysqli, "SELECT * FROM freeboard where NUM=$_POST[id]");
  ...
  <body class="nav-md">
    ...
    function postForm() {
              ...
              try{
                var url ='../board/freeboard_resubmit.php';
                ...
                $.ajax({
                  ...
                  success: function(res) {
                    if(res)
                    {
                      ...
                      if(result[0] == "permission_error")
                      {
                        ...
                        document.getElementById("signerrormessage").innerHTML = result[1]+" 파일의 크기가 제한용량을 초과하였습니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "length_error file"){
                        document.getElementById("signerrormessage").innerHTML = result[1]+" 파일의 제목이 제한길이를 초과하였습니다.";
                        $('#signuperror').modal();
                      }
                      else if(result[0] == "filetype_error file"){
                        document.getElementById("signerrormessage").innerHTML = result[1]+"은(는) 허용되지 않는 확장자 입니다.";
                        $('#signuperror').modal();
                      }
                  ...
                  error: function(request,status,error) {
                    document.getElementById("signerrormessage").innerHTML = "code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error;
                    $('#signuperror').modal();
                    return false;
                    }
                  });
