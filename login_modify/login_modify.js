...
$(document).ready(function($) {
            ...
            // 공백입력방지
            $("#pwd, #email").bind("keypress", function(e) {
...
/*	$("#username").blur(function(){
		...
		if (bytes < 2 || bytes > 30) {
			...
			oMsg_2.innerHTML = "이름은 최대 15자(30 Byte)이내로 한글/영문/숫자 조합만 가능합니다.";
...
function chkPasswordNew(pwd)
{
	...
	{
    documnet.getElementById("signerrormessage").innerHTML = "비밀번호는 띄어쓰기 없이 6~15자의 영문 대/소문자, 숫자\n및 특수문자 2가지 이상 조합으로 입력하셔야 합니다.";
		$("#signuperror").modal();
		$("#pwd1").val("");
		$("#pwd1").focus();
		return false;
...
*/
  ...
  */
	$('#form_main').attr('action', 'modify_action.php').submit();
